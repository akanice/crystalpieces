...
class Home extends MY_Controller {
    ...
    public function ipn() {
		$this->input->post('payer_email');
		$this->load->model('configsmodel');
		$data = array(
			'name'	=> $this->input->post('payer_email'),
			'term'		=> $this->input->post('address_name'),
			'term_id'	=> $this->input->post('address_country'),
			'value'		=> $this->input->post('address_city'),
		);
		$product_id = $this->configsmodel->create($data);
		$this->data['temp'] = 'frontend/home/index';
		$this->load->view('frontend/index', $this->data);
    }
